...
export default function NewProductPage() {
  ...
  // por esa combinación de iniciales.
  ...
  const loadDependencies = async () => {
    ...
    const { data: txs, error: err4 } = await supabase
      .from('taxes')
    ...
    const activeTaxes = txs || [];
    setTaxes(activeTaxes);
    const ivaTax = activeTaxes.find((t) => t.name === 'IVA');
    if (ivaTax) {
      setForm((f) => ({ ...f, selected_tax_ids: [ivaTax.id] }));
    }
  };
